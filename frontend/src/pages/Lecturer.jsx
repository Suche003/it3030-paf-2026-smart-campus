// frontend/src/pages/Lecturer.jsx
...
function Lecturer() {
  ...
  // Readiness badge (to show if technician marked as ready)
  const getReadinessBadge = (booking) => {
    if (booking.readiness_status === 'READY') {
      return <span className="readiness-badge ready">🔧 Ready</span>;
    }
    return null;
  };
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      // Determine dot color (precedence: pending > approved > rejected)
      let dotColor = '#aaa';
      if (hasBookings) {
    ...
    <div className="lecturer-dashboard">
      ...
      <div className="filters-bar">
        <div className="status-filters">
          ...
          <button className={filterStatus === 'APPROVED' ? 'active' : ''} onClick={() => setFilterStatus('APPROVED')}>
            ✅ Approved ({stats.approved})
          </button>
          <button className={filterStatus === 'REJECTED' ? 'active' : ''} onClick={() => setFilterStatus('REJECTED')}>
          ...
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {selectedBooking.rejectReason && (
                <p><strong>Rejection Reason:</strong> {selectedBooking.rejectReason}</p>
              )}
              {selectedBooking.readiness_status === 'READY' && (
                <p><strong>🔧 Resource Status:</strong> Ready for use</p>
              )}
            </div>
